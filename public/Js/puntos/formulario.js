        ...
        if (jornadas_punto.length > 0)
        {
            $.each(jornadas_punto, function(i, e)
            {
                ...
                if(e && !$('#table-jornadas tr[data-id="'+e+'"]').length)
                {
                    ...
                    $('#table-jornadas').append('<tr data-id="'+e+'"><td class="index"></td><td>'+option.text()+'</td><td><a href="#" class="btn btn-default btn-sm" data-role="eliminar"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td></tr>');
                    option.remove();
                }
            });
        }
